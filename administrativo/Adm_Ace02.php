...
if (isset($_POST['login'])) {
  $colname_cs_acesso = $_POST['login'];
}
mysql_select_db($database_rec01, $rec01);
$query_cs_acesso = sprintf("SELECT id_ace, usu_data, usu_id, login, senha, perfil_id, situacao_ace, cadastrador_ace, id_usu, nome_usu, id_pf, nome_pf, id_situa, tipo_situa FROM acesso, usuario, perfil, situacao WHERE login LIKE %s AND id_usu=usu_id AND id_pf=perfil_id AND id_situa=situacao_ace ORDER BY login ASC", GetSQLValueString("%" . $colname_cs_acesso . "%", "text"));
$cs_acesso = mysql_query($query_cs_acesso, $rec01) or die(mysql_error());
							...
							<div class="col-md-6 fet-pad">
								<div class="div-margin">
									...
									<div class="col-md-9 fet-pad wid2">
                                       ...
                                       Informe o Nome: <input name="login" type="text" size="45" maxlength="20">
                                       <input type="submit" name="ok" id="ok" value="Consultar">
                                       </form>
                                      <br>
                                      <h4>Resultado..:</h4>
                                      <?php
                                         $res=$totalRows_cs_acesso;
                                            if($res>0)
											{?>
                                       <?php do { ?>
                                       <table width="100%" border="1" cellspacing="10" cellpadding="10">
                                         <tr>                                          
                                           ...
                                           <td><?php echo $row_cs_acesso['nome_usu']; ?></td>
                                         </tr>
                                         <tr>
                                           <th scope="row">Login </th>
                                           <td><?php echo $row_cs_acesso['login']; ?></td>
                                         </tr>
                                         <tr>
                                           <th scope="row">Perfil</th>
                                           <td><?php echo $row_cs_acesso['nome_pf']; ?></td>
                                         </tr>
                                         <tr>
                                           <th scope="row">Situação</th>
                                           <td><?php echo $row_cs_acesso['tipo_situa']; ?></td>
                                         </tr>
                                         <tr>
                                       ...
                                       <?php } while ($row_cs_acesso = mysql_fetch_assoc($cs_acesso)); ?>
                                       <?php } else {			   
